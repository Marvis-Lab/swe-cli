"""ACE Processor for handling Agent-Computer Interface components."""
...
class ACEProcessor:
    """Handles ACE (Agent-Computer Interface) components and logic."""
    ...
    def __init__(self, session_manager: "SessionManager"):
        ...
        Args:
            session_manager: Session manager for playbook access
        """
    ...
    def ensure_initialized(self, agent) -> None:
    ...
    def set_last_agent_response(self, content: str, tool_calls: list) -> None:
        """Set the last agent response for reflection.

        Args:
            content: Text content from agent
            tool_calls: Tool calls from agent
        """
        ...
        try:
            ...
            if self._ace_reflector:
                ...
                if self._ace_curator:
                    ...
                    # Debug logging
                    self._log_evolution(
                        query, outcome, bullets_before, bullets_after,
                        curator_output, reflection
                    )
    ...
    def _log_evolution(
        self, query, outcome, bullets_before, bullets_after, curator_output, reflection
    ):
        """Log ACE playbook evolution."""
        if bullets_after != bullets_before or curator_output.delta.operations:
            ...
            with open(self.PLAYBOOK_DEBUG_PATH, "a", encoding="utf-8") as log:
                ...
                for op in curator_output.delta.operations:
                    content = op.content[:80] if op.content else op.bullet_id
                    log.write(f"  - {op.type}: {op.section} - {content}\n")
                log.write(f"Reflection Key Insight: {reflection.key_insight}\n")
                reasoning = curator_output.delta.reasoning[:200]
                log.write(f"Curator Reasoning: {reasoning}\n")
    ...
    def _log_error(self, error_msg: str):
        ...
        with open(self.PLAYBOOK_DEBUG_PATH, "a", encoding="utf-8") as log:
            log.write(f"\n{'!' * 60}\n")
            log.write(f"❌ ACE ERROR: {error_msg}\n")
