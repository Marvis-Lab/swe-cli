...
from swecli.ui_textual.widgets.conversation.renderers.nested_tool_renderer import (
    NestedToolRenderer,
)
from swecli.ui_textual.widgets.conversation.renderers.parallel_agent_renderer import (
    ParallelAgentRenderer,
)
from swecli.ui_textual.widgets.conversation.renderers.single_agent_renderer import (
    SingleAgentRenderer,
)
...
class DefaultToolRenderer:
    ...
    Delegates responsibility to specialized renderers:
    - ParallelAgentRenderer: Parallel agent groups
    - SingleAgentRenderer: Single agent execution
    - NestedToolRenderer: Nested tool calls and tree display
    """
    ...
    def __init__(self, log: RichLogInterface, app_callback_interface: Any = None):
        ...
        # Animation timer for nested/parallel/single agents
    ...
    def add_nested_tool_call(
        ...
        # Handle Single Agent updates
        if self.single_renderer.has_active_agent():
            self.single_renderer.update_tool_call(tool_text)
            return

        # Handle Parallel Agent updates
        should_render = True
        if self.parallel_renderer.has_active_group():
            should_render = self.parallel_renderer.update_tool_call(parent, tool_text)

        # Render the nested tool line if needed
        if should_render:
            self.nested_renderer.add_nested_tool_call(
                display, depth, parent, tool_id, is_last
            )
    ...
    def complete_nested_tool_call(
        ...
        self.nested_renderer.complete_nested_tool_call(
            tool_name, depth, parent, success, tool_id
        )

        # Stop timers only if no more active tools
        if not self.nested_renderer.has_active_tools():
            if self._nested_tool_timer:
                self._nested_tool_timer.stop()
                self._nested_tool_timer = None
            if self._nested_tool_thread_timer:
    ...
    def _animate_nested_tool_spinner(self) -> None:
        ...
        interval = 0.15
        self._nested_tool_timer = self.log.set_timer(
            interval, self._animate_nested_tool_spinner
        )
        self._nested_tool_thread_timer = threading.Timer(
            interval, self._on_nested_tool_thread_tick
        )
    ...
    def _on_nested_tool_thread_tick(self) -> None:
        ...
        if (
            not self.nested_renderer.has_active_tools()
            and not self.parallel_renderer.has_running_agents()
            and not self.single_renderer.has_active_agent()
        ):
            return
        try:
            if self.app:
    ...
    def on_parallel_agents_start(self, agent_infos: List[dict]) -> None:
        ...
        self.parallel_renderer.start(agent_infos)
        self._start_nested_tool_timer()
    ...
    # --- Nested Tool Result Display Methods (Delegated) ---
