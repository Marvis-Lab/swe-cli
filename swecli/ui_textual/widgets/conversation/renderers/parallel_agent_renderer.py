...
class ParallelAgentRenderer:
    """Renderer for parallel agent execution."""
    ...
    def update_agent_tool(self, tool_call_id: str, tool_name: str) -> None:
        """Update an agent's status when it calls a tool."""
    ...
    def toggle_expansion(self) -> bool:
        ...
        if self.group:
            self.group.expanded = self.expanded
    ...
    def has_running_agents(self) -> bool:
        if self.group is None:
            return False
        return any(a.status == "running" for a in self.group.agents.values())
