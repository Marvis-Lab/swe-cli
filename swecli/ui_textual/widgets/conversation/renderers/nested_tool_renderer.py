...
class NestedToolRenderer:
    ...
    def has_active_tools(self) -> bool:
        ...
        return bool(self._nested_tools) or (
            self._nested_tool_line is not None and self._nested_tool_text is not None
        )
    ...
    def complete_nested_tool_call(
        ...
        if state is None:
            # Fallback: find most recent for this parent
        ...
        # Clear legacy state if it matches the completed tool
        if self._nested_tool_line == state.line_number:
            self._nested_tool_line = None
            self._nested_tool_text = None
