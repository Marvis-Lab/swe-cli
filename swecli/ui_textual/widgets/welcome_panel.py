...
class AnimatedWelcomePanel(Widget):
    ...
    def _calculate_rain_size(self) -> tuple[int, int]:
        """Calculate rain field dimensions based on terminal size."""
        term_width = self.size.width if self.size.width > 0 else 100
        term_height = self.size.height if self.size.height > 0 else 30
        width = max(40, min(90, int(term_width * 0.7)))
        height = max(10, min(20, term_height - 10))
        return width, height
    ...
    def render(self) -> RenderableType:
        """Render the animated welcome panel: rain field on top, text below."""
        ...
        # Stack vertically: rain on top, text bar below
        content = Group(
            Align.center(rain),
            Text(""),
