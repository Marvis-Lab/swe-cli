...
- File-based output storage (/tmp/swe-cli/<path>/tasks/<id>.output)
...
class BackgroundTaskManager:
    ...
    def _get_output_dir(self) -> Path:
        """Get output directory following Claude Code pattern.

        Returns:
            Path like /tmp/swe-cli/-Users-nghibui-codes-test-opencli/tasks/
        """
        # Convert path to safe directory name (replace / with -)
        cwd_str = str(self.working_dir.resolve())
        safe_path = cwd_str.replace("/", "-")
        return Path(f"/tmp/swe-cli/{safe_path}/tasks")
