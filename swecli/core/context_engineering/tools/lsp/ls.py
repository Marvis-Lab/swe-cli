...
class SolidLanguageServer(ABC):
    ...
    def is_ignored_path(
        ...
        :param is_dir: Optimization: Pass True if path is known to be a directory
        :param is_file: Optimization: Pass True if path is known to be a file
    ...
    def request_full_symbol_tree(self, within_relative_path: str | None = None) -> list[ls_types.UnifiedSymbolInformation]:
        ...
        def process_directory(rel_dir_path: str) -> list[ls_types.UnifiedSymbolInformation]:
            ...
            if self.is_ignored_path(
                str(Path(abs_dir_path).relative_to(self.repository_root_path)), is_dir=True
            ):
            ...
            try:
                ...
                with os.scandir(abs_dir_path) as it:
                    # scandir returns an iterator, so we can't get sorted names directly
                    # If deterministic order is needed, we might need to sort, but let's assume not strict for now
                    # or better: collect and sort
                    ...
                    for entry in entries:
                        ...
                        try:
                            # Avoid full resolve if possible for performance, but stick to safe relative calc for now
                        ...
                        if self.is_ignored_path(
                            contained_dir_or_file_rel_path, is_dir=is_dir, is_file=is_file
                        ):
                            log.debug(
