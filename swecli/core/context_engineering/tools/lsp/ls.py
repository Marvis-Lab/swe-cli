...
class SolidLanguageServer(ABC):
    ...
    def __init__(
        ...
        self.cache = LanguageServerCache(
            repository_root_path=self.repository_root_path,
            project_data_relative_path=self._solidlsp_settings.project_data_relative_path,
            language_id=self.language_id,
            ls_specific_raw_document_symbols_cache_version=cache_version_raw_document_symbols,
        )
    ...
    def _request_document_symbols(
        ...
        def get_cached_raw_document_symbols(cache_key: str, fd: LSPFileBuffer) -> list[SymbolInformation] | list[DocumentSymbol] | None:
            file_hash_and_result = self.cache.raw_document_symbols_cache.get(cache_key)
        ...
        def get_raw_document_symbols(fd: LSPFileBuffer) -> list[SymbolInformation] | list[DocumentSymbol] | None:
            ...
            # update cache
            self.cache.raw_document_symbols_cache[cache_key] = (fd.content_hash, response)
            self.cache.raw_document_symbols_cache_is_modified = True
    ...
    def request_document_symbols(self, relative_file_path: str, file_buffer: LSPFileBuffer | None = None) -> DocumentSymbols:
        ...
        with self._open_file_context(relative_file_path, file_buffer) as file_data:
            ...
            cache_key = relative_file_path
            file_hash_and_result = self.cache.document_symbols_cache.get(cache_key)
            if file_hash_and_result is not None:
            ...
            self.cache.document_symbols_cache[cache_key] = (file_data.content_hash, document_symbols)
            self.cache.document_symbols_cache_is_modified = True
