...
class SymbolCacheManager:
    ...
    def __init__(self, cache_dir: Path, raw_cache_version: Hashable = 1):
        self.cache_dir = cache_dir
        self.cache_dir.mkdir(parents=True, exist_ok=True)

        self._ls_specific_raw_document_symbols_cache_version = raw_cache_version
        ...
        if not cache_file.exists():
            ...
            if legacy_cache_file.exists():
                try:
                    ...
                    ] = load_pickle(str(legacy_cache_file)) # compat.load_pickle expects path string
                    if legacy_cache:
    ...
    def get_raw_document_symbols(
        self, relative_path: str, file_hash: str
    ) -> list[SymbolInformation] | list[DocumentSymbol] | None:
        file_hash_and_result = self._raw_document_symbols_cache.get(relative_path)
        if file_hash_and_result is not None:
            cached_file_hash, result = file_hash_and_result
            if cached_file_hash == file_hash:
                return result
        return None

    def update_raw_document_symbols(
        self,
        relative_path: str,
        file_hash: str,
        symbols: list[SymbolInformation] | list[DocumentSymbol] | None
    ) -> None:
        self._raw_document_symbols_cache[relative_path] = (file_hash, symbols)
        self._raw_document_symbols_cache_is_modified = True

    def get_document_symbols(self, relative_path: str, file_hash: str) -> DocumentSymbols | None:
        file_hash_and_result = self._document_symbols_cache.get(relative_path)
        if file_hash_and_result is not None:
            cached_file_hash, document_symbols = file_hash_and_result
            if cached_file_hash == file_hash:
                return document_symbols
        return None

    def update_document_symbols(self, relative_path: str, file_hash: str, symbols: DocumentSymbols) -> None:
        self._document_symbols_cache[relative_path] = (file_hash, symbols)
