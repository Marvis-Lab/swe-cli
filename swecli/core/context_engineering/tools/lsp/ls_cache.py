...
class LSCacheMixin:
    """
    Mixin for managing document symbol caches.
    Assumes that the consuming class has `cache_dir` (Path) and `RAW_DOCUMENT_SYMBOL_CACHE_FILENAME` etc.
    However, to make it cleaner, we can define the constants here or abstract them.
    The original code had constants on SolidLanguageServer.
    """
    ...
    def _init_cache(self, cache_dir: Path, ls_specific_raw_document_symbols_cache_version: Hashable) -> None:
        ...
        self._raw_document_symbols_cache: dict[
            str, tuple[str, list[LSPTypes.DocumentSymbol] | list[LSPTypes.SymbolInformation] | None]
        ] = {}
