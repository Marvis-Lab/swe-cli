...
if TYPE_CHECKING:
    from swecli.core.context_engineering.tools.lsp.ls import LSPFileBuffer
...
class LanguageServerCache:
    ...
    def __init__(
        ...
        self.cache_dir = (
            Path(self.repository_root_path) / project_data_relative_path / self.CACHE_FOLDER_NAME / language_id
        )
    ...
    def get_raw_document_symbols(self, cache_key: str, fd: "LSPFileBuffer") -> list[SymbolInformation] | list[DocumentSymbol] | None:
    ...
    def update_raw_document_symbols(self, cache_key: str, fd: "LSPFileBuffer", response: list[SymbolInformation] | list[DocumentSymbol] | None) -> None:
        self._raw_document_symbols_cache[cache_key] = (fd.content_hash, response)
        self._raw_document_symbols_cache_is_modified = True

    def get_document_symbols(self, cache_key: str, fd: "LSPFileBuffer") -> DocumentSymbols | None:
    ...
    def update_document_symbols(self, cache_key: str, fd: "LSPFileBuffer", document_symbols: DocumentSymbols) -> None:
        log.debug("Updating cached document symbols for %s", cache_key)
        self._document_symbols_cache[cache_key] = (fd.content_hash, document_symbols)
