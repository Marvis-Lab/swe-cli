...
class LanguageServerCache:
    """
    Handles caching of raw document symbols and high-level document symbols for the Language Server.
    """
    ...
    def __init__(self, cache_dir: Path, ls_specific_raw_document_symbols_cache_version: Hashable):
    ...
    def get_raw_document_symbols(self, cache_key: str, content_hash: str) -> list[DocumentSymbol] | list[SymbolInformation] | None:
    ...
    def set_raw_document_symbols(self, cache_key: str, content_hash: str, symbols: list[DocumentSymbol] | list[SymbolInformation] | None) -> None:
        self._raw_document_symbols_cache[cache_key] = (content_hash, symbols)
        self._raw_document_symbols_cache_is_modified = True

    def get_document_symbols(self, cache_key: str, content_hash: str) -> DocumentSymbols | None:
    ...
    def set_document_symbols(self, cache_key: str, content_hash: str, document_symbols: DocumentSymbols) -> None:
    ...
    def _load_raw_document_symbols_cache(self) -> None:
        ...
        if not cache_file.exists():
            ...
            if legacy_cache_file.exists():
                try:
                    legacy_cache: dict[
                        str, tuple[str, tuple[list[UnifiedSymbolInformation], list[UnifiedSymbolInformation]]]
