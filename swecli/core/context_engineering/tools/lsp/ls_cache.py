...
class LanguageServerCache:
    """
    Manages caching for Language Server symbols.
    """
    ...
    def __init__(self, cache_dir: Path, ls_specific_raw_document_symbols_cache_version: Hashable = 1):
    ...
    def get_raw_document_symbols(self, cache_key: str) -> tuple[str, list[DocumentSymbol] | list[SymbolInformation] | None] | None:
        return self._raw_document_symbols_cache.get(cache_key)

    def set_raw_document_symbols(
        self, cache_key: str, value: tuple[str, list[DocumentSymbol] | list[SymbolInformation] | None]
    ) -> None:
        self._raw_document_symbols_cache[cache_key] = value
        self._raw_document_symbols_cache_is_modified = True

    def get_document_symbols(self, cache_key: str) -> tuple[str, DocumentSymbols] | None:
        return self._document_symbols_cache.get(cache_key)

    def set_document_symbols(self, cache_key: str, value: tuple[str, DocumentSymbols]) -> None:
        self._document_symbols_cache[cache_key] = value
