...
class DockerSubAgentExecutor:
    ...
    def extract_github_info(self, task: str) -> tuple[str, str, str] | None:
...
"""
    ...
    def execute_with_docker_handler(
        ...
        if description is None:
            description = self.extract_task_description(task)
    ...
    def extract_task_description(self, task: str) -> str:
    ...
    def execute_with_docker(
        ...
        local_working_dir = local_output_dir or (Path(self.manager._working_dir) if self.manager._working_dir else Path.cwd())
        ...
        try:
            ...
            if name == "GitHub-Resolver":
                github_info = self.extract_github_info(task)
                if github_info:
